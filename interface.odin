...
interface_edit_entity :: proc(game : ^game_state, curr_room : ^room) {
		...
		case .VERTICES:
			{
				if def.shape_type == .capsuleShape {
				   ...
				   def.shape_type == .chainSegmentShape {
					if ion_is_pressed(.MOUSE_RIGHT) {
						...
						small_array.set(
							&def.vertices,
							game.selected_vertex_index,
							mpos,
						)
						level_reload(game, curr_room)

					} else if ion_is_pressed(.DELETE) {
						small_array.unordered_remove(
							&def.vertices,
							game.selected_vertex_index,
						)
...
interface_handle_input :: proc(game : ^game_state) {
	...
	if game.edit_mode == .VERTICES {
		...
		if state.input.mouse_wheel_y != 0 {
			...
			if def.shape_type == .polygonShape ||
			   def.shape_type == .chainSegmentShape {
...
interface_body_def_editor :: proc(game : ^game_state, curr_room : ^room) {
	...
	im.SliderFloat2("Position", &def.body_def.position, -500, 500)
	im.InputFloat2("Position_", &def.body_def.position)
...
interface_shape_def_editor :: proc(game : ^game_state, curr_room : ^room) {
	...
	if im.TreeNode("Events and contacts") {
		...
		im.Checkbox(
			...
			&def.shape_def.enableContactEvents,
		)
		im.Checkbox("Enable Hit Events", &def.shape_def.enableHitEvents)
		im.Checkbox(
			"Enable Presolve Events",
			&def.shape_def.enablePreSolveEvents,
		)
		im.Checkbox(
			"Invoke contact Creation",
			&def.shape_def.invokeContactCreation,
		)
	...
	if im.TreeNode("Material") {
		...
		im.SliderFloat(
			"Restitution",
			&def.shape_def.material.restitution,
			0,
			1,
		)
		im.SliderFloat(
			"Rolling Resistance",
			&def.shape_def.material.rollingResistance,
			0,
			1,
		)
		im.SliderFloat(
			"Tangent Speed",
			&def.shape_def.material.tangentSpeed,
			0,
			1,
		)
	...
	if im.TreeNode("Filter") {
		...
		for type in entity_type {
			contains := bool(u64(type) & def.shape_def.filter.categoryBits)
			if im.Checkbox(fmt.ctprint(type), &contains) do def.shape_def.filter.categoryBits ~= u64(type)
		}

		im.Text("Mask Bits")
		for type in entity_type {
			contains := bool(u64(type) & def.shape_def.filter.maskBits)
			if im.Checkbox(fmt.ctprint(" ", type), &contains) {
				def.shape_def.filter.maskBits ~= u64(type)
				level_reload(game, curr_room)
			}
		}

		im.InputInt("Group index", &def.shape_def.filter.groupIndex)
		im.Separator()
		im.TreePop()
	}
}
...
interface_edit_level :: proc(game : ^game_state) {
	if im.BeginTabItem("Levels", nil, {.Leading}) {
		...
		if curr_level != nil && curr_level.initilized {
			...
			im.InputText(
				"New room name",
				cstring(&curr_level.new_room_buff[0]),
				255,
			)
				...
				if val.initilized {
					...
					if len(val.rooms) > 0 {
						for room_name in val.rooms {
							if im.Selectable(
								fmt.ctprint(room_name),
								game.curr_level == level_name &&
								val.curr_room == room_name,
							) {
								game.selected_index = -1
								game.curr_level = level_name
...
interface_edit_room :: proc(game : ^game_state) {
	...
	if im.BeginTabItem("Room", nil) {
		im.SliderFloat("Zoom", &curr_room.zoom, 0, 100)

		im.Text("Room name %s", curr_room.name)
		im.Text("World id %d", curr_room.world_id.index1)

		for flag in room_flags_enum {
			contains := flag in curr_room.flags
			if im.Checkbox(fmt.ctprint(flag), &contains) do curr_room.flags ~= {flag}
		}
...
interface_static_index_editor :: proc(
	game : ^game_state,
	curr_room : ^room,
	def : ^entity_def,
	curr_entity : ^entity,
	...
	if indexes != nil {
		for val, i in indexes {
			im.Text(
				"%s, %s index = %d",
				fmt.ctprint(val.level),
				fmt.ctprint(val.room),
				val.index,
			)
...
interface_edit_entity_ui :: proc(game : ^game_state) {
	...
	if im.BeginTabItem("Entity", nil, {.Leading}) {
		...
		if game.selected_index != -1 {
			...
			entity := &curr_room.entities[game.selected_index]
			def_old := curr_room.entity_defs[game.selected_index]
			def := &curr_room.entity_defs[game.selected_index]
	...
	if game.edit_mode == .INSERT {
		if ion_is_pressed(.MOUSE_RIGHT) {
			mpos : [2]f32 = {
				f32(state.input.mouse_x),
				f32(state.input.mouse_y),
			}
			mpos = camera_convert_screen_to_world(&state.draw.cam, mpos)
