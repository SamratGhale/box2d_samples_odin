...
level_reload :: proc(game : ^game_state, curr_room : ^room) {

	clear(&curr_room.entities)

	if curr_room.world_id != b2.nullWorldId {
		b2.DestroyWorld(curr_room.world_id)
	}

	{
		...
		curr_room.world_id = b2.CreateWorld(world_def)
	}

	clear(&curr_room.static_indexes)

	for def, i in curr_room.entity_defs {
		new_entity := entity_create_new(def, curr_room.world_id, i32(i))

		append(&curr_room.entities, new_entity)
...
level_get_all :: proc "c" (
	game : ^game_state,
	index : ^static_index_global,
) -> (
	^level,
	^room,
	^entity,
