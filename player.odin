...
*/
player_collision_callback :: proc "c" (
	shape_id : b2.ShapeId,
	ctx : rawptr,
) -> bool {
	...
	case .KEY:
		{
			b2.Body_Disable(b2.Shape_GetBody(shape_id))
...
player_update_rotation :: proc(
	...
	if gaps & player.flags != {} {
		...
		b2.Body_SetTransform(
			player.body_id,
			pos,
			b2.Body_GetRotation(player.body_id),
		)
...
*/
player_get_rotated_asdw :: proc(
	curr_room : ^room,
) -> (
	asdwjump : [5]ion_button,
) {
...
player_update_jump :: proc(
	...
	if .JUMPING not_in player.flags {
		player.flags += {.GAP_LEFT, .GAP_RIGHT}

		if curr_room.ground_id != b2.nullShapeId {
			floor_index := u32(
				uintptr(b2.Shape_GetUserData(curr_room.ground_id)),
			)
...
player_update_bounding_box :: proc(
	game : ^game_state,
	curr_room : ^room,
	player : ^entity,
) {
	if curr_room.ground_id != b2.nullShapeId {
		...
		aabb_left, aabb_right := player_get_bounding_box(
			state.draw.cam.rotation,
			pos,
		)
		...
		result := b2.World_OverlapAABB(
			curr_room.world_id,
			aabb_right,
			bounding_box_filter,
			overlap_callback_right,
			game,
		)
		result = b2.World_OverlapAABB(
			curr_room.world_id,
