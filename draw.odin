...
camera_get_view_bounds :: proc(cam : ^Camera) -> b2.AABB {
	return b2.AABB {
		lowerBound = camera_convert_screen_to_world(cam, {0, f32(cam.height)}),
		upperBound = camera_convert_screen_to_world(cam, {f32(cam.width), 0}),
	}
}
...
background_draw :: proc(back : ^Background, cam : ^Camera) {
	...
	gl.Uniform3f(back.uniforms["baseColor"].location, 0.8, 0.9, 0.2)
